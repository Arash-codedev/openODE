...
/* Path: ./auto-coded/systems/actsys/solver.cpp   */
...
bool actSys_solver::less_with_sign(double t1,double t2,double dt)
{
	if(dt>0.0)
		//return t1 < t2;
...
bool actSys_solver::less_eq_with_sign(double t1,double t2,double dt)
{
	if(dt>0.0)
		return t1-t2 <= epsilon;
	else
...
void actSys_solver::plant_do_step(
		actSys_manager &usersystem,
		position &plant_x,
		double &t,
		double &dt)
{
...
void actSys_solver::plant_do_step2(
		actSys_manager &usersystem,
		const position &x_in,
...
double actSys_solver::plant_error(const position &plant_x_old,const position &plant_dxdt_old,position &plant_x_err,double dt) const
{
...
actSys_solver::actSys_solver(actSys_manager &usersystem):
...
std::size_t actSys_solver::integrate(
