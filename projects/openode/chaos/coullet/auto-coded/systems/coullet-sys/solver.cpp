...
/* Path: ./auto-coded/systems/coullet-sys/solver.cpp */
...
bool CoulletSys_solver::less_with_sign(double t1,double t2,double dt)
{
	if(dt>0.0)
		//return t1 < t2;
...
bool CoulletSys_solver::less_eq_with_sign(double t1,double t2,double dt)
{
	if(dt>0.0)
		return t1-t2 <= epsilon;
	else
...
void CoulletSys_solver::plant_do_step(
		CoulletSys_manager &usersystem,
		position &plant_x,
		double &t,
		double &dt)
{
...
void CoulletSys_solver::plant_do_step2(
		CoulletSys_manager &usersystem,
		const position &x_in,
...
double CoulletSys_solver::plant_error(const position &plant_x_old,const position &plant_dxdt_old,position &plant_x_err,double dt) const
{
...
CoulletSys_solver::CoulletSys_solver(CoulletSys_manager &usersystem):
...
std::size_t CoulletSys_solver::integrate(
