...
/* Path: ./auto-coded/systems/lorenzsys/solver.cpp */
...
bool LorenzSys_solver::less_with_sign(double t1,double t2,double dt)
{
	if(dt>0.0)
		//return t1 < t2;
...
bool LorenzSys_solver::less_eq_with_sign(double t1,double t2,double dt)
{
	if(dt>0.0)
		return t1-t2 <= epsilon;
	else
...
void LorenzSys_solver::plant_do_step(
		LorenzSys_manager &usersystem,
		position &plant_x,
		double &t,
		double &dt)
{
...
void LorenzSys_solver::plant_do_step2(
		LorenzSys_manager &usersystem,
		const position &x_in,
...
double LorenzSys_solver::plant_error(const position &plant_x_old,const position &plant_dxdt_old,position &plant_x_err,double dt) const
{
...
LorenzSys_solver::LorenzSys_solver(LorenzSys_manager &usersystem):
...
std::size_t LorenzSys_solver::integrate(
