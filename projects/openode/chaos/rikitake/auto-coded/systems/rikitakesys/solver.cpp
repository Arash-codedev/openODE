...
/* Path: ./auto-coded/systems/rikitakesys/solver.cpp */
...
bool RikitakeSys_solver::less_with_sign(double t1,double t2,double dt)
{
	if(dt>0.0)
		//return t1 < t2;
...
bool RikitakeSys_solver::less_eq_with_sign(double t1,double t2,double dt)
{
	if(dt>0.0)
		return t1-t2 <= epsilon;
	else
...
void RikitakeSys_solver::plant_do_step(
		RikitakeSys_manager &usersystem,
		position &plant_x,
		double &t,
		double &dt)
{
...
void RikitakeSys_solver::plant_do_step2(
		RikitakeSys_manager &usersystem,
		const position &x_in,
...
double RikitakeSys_solver::plant_error(const position &plant_x_old,const position &plant_dxdt_old,position &plant_x_err,double dt) const
{
...
RikitakeSys_solver::RikitakeSys_solver(RikitakeSys_manager &usersystem):
...
std::size_t RikitakeSys_solver::integrate(
