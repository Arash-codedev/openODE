...
/* Path: ./auto-coded/systems/rosslersys/solver.cpp */
...
bool RosslerSys_solver::less_with_sign(double t1,double t2,double dt)
{
	if(dt>0.0)
		//return t1 < t2;
...
bool RosslerSys_solver::less_eq_with_sign(double t1,double t2,double dt)
{
	if(dt>0.0)
		return t1-t2 <= epsilon;
	else
...
void RosslerSys_solver::plant_do_step(
		RosslerSys_manager &usersystem,
		position &plant_x,
		double &t,
		double &dt)
{
...
void RosslerSys_solver::plant_do_step2(
		RosslerSys_manager &usersystem,
		const position &x_in,
...
double RosslerSys_solver::plant_error(const position &plant_x_old,const position &plant_dxdt_old,position &plant_x_err,double dt) const
{
...
RosslerSys_solver::RosslerSys_solver(RosslerSys_manager &usersystem):
...
std::size_t RosslerSys_solver::integrate(
