...
/* Path: ./auto-coded/systems/thomas-sys/solver.cpp */
...
bool ThomasSys_solver::less_with_sign(double t1,double t2,double dt)
{
	if(dt>0.0)
		//return t1 < t2;
...
bool ThomasSys_solver::less_eq_with_sign(double t1,double t2,double dt)
{
	if(dt>0.0)
		return t1-t2 <= epsilon;
	else
...
void ThomasSys_solver::plant_do_step(
		ThomasSys_manager &usersystem,
		position &plant_x,
		double &t,
		double &dt)
{
...
void ThomasSys_solver::plant_do_step2(
		ThomasSys_manager &usersystem,
		const position &x_in,
...
double ThomasSys_solver::plant_error(const position &plant_x_old,const position &plant_dxdt_old,position &plant_x_err,double dt) const
{
...
ThomasSys_solver::ThomasSys_solver(ThomasSys_manager &usersystem):
...
std::size_t ThomasSys_solver::integrate(
