...
/* Path: ./auto-coded/systems/windmi-sys/solver.cpp */
...
bool WINDMISys_solver::less_with_sign(double t1,double t2,double dt)
{
	if(dt>0.0)
		//return t1 < t2;
...
bool WINDMISys_solver::less_eq_with_sign(double t1,double t2,double dt)
{
	if(dt>0.0)
		return t1-t2 <= epsilon;
	else
...
void WINDMISys_solver::plant_do_step(
...
void WINDMISys_solver::plant_do_step2(
		WINDMISys_manager &usersystem,
		const position &x_in,
...
double WINDMISys_solver::plant_error(const position &plant_x_old,const position &plant_dxdt_old,position &plant_x_err,double dt) const
{
...
WINDMISys_solver::WINDMISys_solver(WINDMISys_manager &usersystem):
...
std::size_t WINDMISys_solver::integrate(
