...
void Universegraph::universe_display(double t,vector<GraphicStar> &stars)
{
	...
	// float cam_angle1_deg=float(t_cam*10.0+20.0);
	// float cam_angle2_deg=float(sin((t_cam+20.0)*0.03))*camera_angle2_max_deg;
	if(auto_center)
	{
		double target_x=0.0;
		double target_y=0.0;
	...
	float cam_x=look_at_x+camera_radius*float(cos(cam_angle1_deg*deg2rad))*float(cos(cam_angle2_deg*deg2rad));
	float cam_y=look_at_y+camera_radius*float(sin(cam_angle1_deg*deg2rad))*float(cos(cam_angle2_deg*deg2rad));
	float cam_z=look_at_z+camera_radius*float(sin(cam_angle2_deg*deg2rad));
