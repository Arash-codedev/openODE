...
void Window::init()
{
	...
	// Fixing initial unstable frames:
	{
	...
	{
		{
			...
			candidate_file=screenshot_folder/(screenshot_prefix+zeros+number+".bmp");
...
{
	//********************************************//
	// template to convert sceenshots to video:   //
	// $ ffmpeg -framerate 30 -i ./screenshots/img-%05d.bmp -s 1280x720 -c:v libx264 -profile:v high -crf 20 -pix_fmt yuv420p output.mp4
	//********************************************//
...
void Window::draw_horizontal_grid()
{
	glLineWidth(1.0);
