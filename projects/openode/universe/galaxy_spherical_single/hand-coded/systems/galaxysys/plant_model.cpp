...
{
	return exp(1.0/3.0*log(mass))*0.025*1.4;
}
...
void GalaxySys_plant_init_states(double /*t*/,Galaxy& states0)
{
	...
	const double mass_min=100.0;
	const double mass_span=500;
	const double mass_avg=mass_min+mass_span/2.0;
	const double galaxy_radius=20.0*4.0;
	for(int i=0;i<N_stars;i++)
	{
		double r=galaxy_radius;
		double kz=1.0;
		Star star;
		double x,y,z;
		do
		{
			x=(2.0*rand01()-1.0)*r;
			y=(2.0*rand01()-1.0)*r;
			z=(2.0*rand01()-1.0)*r*kz;
		} while(x*x+y*y+z*z>r*r);
		star.x=x;
		star.y=y;
		star.z=z;

		star.mass=mass_min+rand01()*mass_span;
		double estimated_center_mass=double(N_stars)*r*r/galaxy_radius/galaxy_radius*mass_avg;
		double vel=sqrt(G*estimated_center_mass/r)*0.25;
		if(std::isnan(vel))
			vel=(rand01()-rand01())*0.01;

		star.vx=+vel*(rand01()-rand01());
		star.vy=+vel*(rand01()-rand01());
		star.vz=+vel*(rand01()-rand01())*kz;

		random_color(star.r,star.g,star.b);
		states0.stars[i]=star;
	}
}
		...
		GalaxySys_manager& system)
{
	...
	universegraph.camera_radius=50.0f*3.0;
	universegraph.transparency_base=4;
	universegraph.auto_center=true;
}
